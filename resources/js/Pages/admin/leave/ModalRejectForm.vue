...
<template>
    <VDialog :showModal="openDialog" title="Reject Leave" @opened="openForm" @closed="closeForm" size="lg">
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                <div class="col-span-2">
                    <VInput placeholder="Insert Reason" label="Reject Reason" :required="true" v-model="form.reject_reason"
                        :errorMessage="formError.reject_reason" @update:modelValue="formError.reject_reason = ''" />
                </div>
            </div>
        </template>
        <template v-slot:footer>
            <div class="flex flex-wrap justify-end space-x-2">
                ...
                <VButton :is-loading="isLoading" label="Submit" type="danger" @click="submit" />
            </div>
        </template>
    </VDialog>
</template>
