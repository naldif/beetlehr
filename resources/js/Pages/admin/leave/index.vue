...
const heads = ["Name", "Start Date", "End Date", "Type", "Status", ""]
...
const deleteEmployee = async () => {
    axios.delete(route('leave.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-between items-center">
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Leave Management</h1>
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200"
        ...
        <header class="block justify-between items-center sm:flex py-6 px-4">
            <h2 class="font-semibold text-slate-800">
                All Leave Submissions <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
            </h2>
            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                ...
                <VButton label="Add Leave" type="primary" @click="handleAddModalForm" class="mt-auto" />
            </div>
        </header>
        ...
        <VDataTable :heads="heads" :isLoading="isLoading">
            ...
            <tr v-for="(data, index) in query" :key="index" v-else>
                ...
                <td class="px-4 whitespace-nowrap h-16"> {{ data.start_date }} </td>
                <td class="px-4 whitespace-nowrap h-16"> {{ data.end_date }} </td>
                <td class="px-4 whitespace-nowrap h-16"> {{ data.leave_type }} </td>
