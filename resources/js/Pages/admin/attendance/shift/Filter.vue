...
<template>
    <!-- Search -->
    <VInput placeholder="Search" is-prefix v-model="searchValue" @update:model-value="search">
    ...
    <VFilter align="right" @apply="applyFilter" @clear="clearFilter">
        <div class="flex px-4 space-x-3 w-full">
            <div class="pt-1.5 pb-4 px-4 ">
                <div class="text-xs font-semibold text-slate-400 uppercase mb-2">Sort By Name</div>
                <VRadio name="minimum-value" v-model="filter.sort_by_name" :options="minimumValueOptions" />
            </div>
            <div class="pt-1.5 pb-4 px-4 ">
                <div class="text-xs font-semibold text-slate-400 uppercase mb-2">Filter by Type</div>
                <VCheckbox v-model="filter.filter_normal_shift" label="Normal Shift" />
                <VCheckbox v-model="filter.filter_night_shift" label="Night Shift" />
            </div>
        </div>
    </VFilter>
</template>