...
const heads = ["Name", "Default Amount", "Status", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.payroll.components.branch.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('settings.payroll.components.branch.delete', { 'id': itemSelected.value.branch_component_id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <section>
        <header class="block justify-between items-center sm:flex pb-6">
            <h2 class="font-semibold text-slate-800">
                All Branch Component <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
            </h2>
