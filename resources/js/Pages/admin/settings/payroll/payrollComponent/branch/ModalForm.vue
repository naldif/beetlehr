...
const updateHandle = async () => {
    ...
    axios.put(route('settings.payroll.components.branch.update'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
<template>
    <VDialog :showModal="openDialog" :title="updateAction ? 'Update Branch Component' : 'Add Payroll Component'"
        ...
        <template v-slot:content>
            <div class="grid grid-cols-1 gap-3">
                <VInput placeholder="Input Branch Name" label="Name" :required="true" v-model="form.name"
                    :errorMessage="formError.name" @update:modelValue="formError.name = ''" :disabled="true" />
                <VInput placeholder="Input Default Amount" label="Default Amount" :required="true"
                    v-model="form.default_amount" :errorMessage="formError.default_amount"
                    @update:modelValue="formError.default_amount = ''" />
                <div>
