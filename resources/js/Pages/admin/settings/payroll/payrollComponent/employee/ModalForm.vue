...
const openForm = () => {
    if(props.updateAction){
        ...
        form.value.component_id = props.additional.id
    }else{
        form.value = ref({})
    }
}
...
const updateHandle = async () => {
    isLoading.value = true
    axios.put(route('settings.payroll.components.employee.update'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
<template>
    <VDialog :showModal="openDialog" :title="updateAction ? 'Update Employee Component' : 'Add Payroll Component'" @opened="openForm" @closed="closeForm" size="xl">
        ...
        <template v-slot:content>
            <div class="grid grid-cols-1 gap-3">
                <VInput placeholder="Input Employee Name" label="Name" :required="true" v-model="form.name"
                    :errorMessage="formError.name" @update:modelValue="formError.name = ''" :disabled="true"/>
                <VInput placeholder="Input Default Amount" label="Default Amount" :required="true" v-model="form.default_amount"
                    :errorMessage="formError.default_amount" @update:modelValue="formError.default_amount = ''" />
                <div>
