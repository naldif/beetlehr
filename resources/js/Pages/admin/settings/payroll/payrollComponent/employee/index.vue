...
const heads = ["Name", "Branch Name", "Default Amount", "Status", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.payroll.components.employee.getdata'), {
        params: {
            page: page,
            search: searchFilter.value,
            component_id: props.additional.id,
...
const deleteHandle = async () => {
    axios.delete(route('settings.payroll.components.employee.delete', { 'id': itemSelected.value.employee_component_id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <section>
        <header class="block justify-between items-center sm:flex pb-6">
            <h2 class="font-semibold text-slate-800">
                All Employee Component <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
            </h2>
            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                <!-- Filter -->
                <VFilter @search="searchHandle" @apply="applyFilter" @clear="clearFilter" :additional="options" />
            </div>
        </header>
        ...
        <VDataTable :heads="heads" :isLoading="isLoading" wrapperClass="!px-0">
            ...
            <tr v-for="(data, index) in query" :key="index" v-else>
                ...
                <td class="px-4 whitespace-nowrap h-16"> {{ data.default_amount_formatted ?
                        'Rp' + data.default_amount_formatted : '-'
                }} </td>
                ...
                <td class="px-4 whitespace-nowrap h-16 text-right">
                    <VDropdownEditMenu class="relative inline-flex r-0" :align="'right'"
                        ...
                        <li class="cursor-pointer hover:bg-slate-100" @click="alertDelete(data)" v-if="data.can_delete">
                            <div class="flex items-center space-x-2 p-3">
                                <span>
                                    <VTrash color="danger" />
                                </span>
