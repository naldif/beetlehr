...
const createHandle = async () => {
    ...
    axios.post(route('settings.payroll.components.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
const updateHandle = async () => {
    isLoading.value = true
    axios.put(route('settings.payroll.components.update', { 'id': props.data.id }), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
<template>
    <VDialog :showModal="openDialog" :title="updateAction ? 'Update Component' : 'Add Component'" @opened="openForm" @closed="closeForm" size="md">
        ...
        <template v-slot:content>
            <!-- Deduction Late -->
            <div class="grid grid-cols-2 gap-3" v-if="(form.is_mandatory && form.custom_attribute.action === 'deduction_late')">
                <div class="col-span-2">
                    <VInput placeholder="Input Late Tolerance" label="Late Tolerance (minute)" :required="true"
                        v-model="form.late_tolerance" :errorMessage="formError.late_tolerance"
            ...
            <!-- Deduction Late -->
            <div class="grid grid-cols-2 gap-3" v-else-if="(form.is_mandatory && form.custom_attribute.action === 'earning_overtime')">
                <VSwitch v-model="form.is_editable" label="Editable" type="block" />
                <VSwitch v-model="form.is_taxable" label="Taxable" type="block" />
            </div>
            ...
            <div class="grid grid-cols-2 gap-3" v-else>
                ...
                <div class="col-span-2">
                    <VSelect placeholder="Select Type" :required="true" v-model="form.type" :options="typeOptions" label="Type"
                        :errorMessage="formError.type" @update:modelValue="formError.type = ''" ref="typeSelectHandle"
