...
const formError = ref({})
const form = ref({})
const employmentStatusSelectHandle = ref()
...
const closeForm = () => {
    form.value = ref({})
    formError.value = ref({})
    employmentStatusSelectHandle.value.clearSelected()
}

const submit = async () => {
    props.updateAction ? updateEmploymentStatus() : createEmploymentStatus()
}

const createEmploymentStatus = async () => {
    isLoading.value = true
    axios.post(route('settings.employee.status.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
const updateEmploymentStatus = async () => {
    isLoading.value = true
    axios.put(route('settings.employee.status.update', { 'id': props.data.id }), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
<template>
    <VDialog :showModal="openDialog" :title="updateAction ? 'Update Employment Status' : 'Add Employment Status'"
        ...
        <template v-slot:content>
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="col-span-2">
                    <VSelect :options="additional.employment_type_list"
                        placeholder="Select Employement Type" :required="true" v-model="form.pkwt_type"
                        label="Employment Type" :errorMessage="formError.pkwt_type" @update:modelValue="formError.pkwt_type = ''"  ref="employmentStatusSelectHandle" />
                </div>
                <div class="col-span-2">
