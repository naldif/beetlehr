...
const breadcrumb = [
    ...
    },
    {
        ...
        active: false,
        to: route('settings.systems.role.editpage', {'id': props.additional.role.id})
    },
    {
...
const form = ref({
    role_name: props.additional.role.name,
    permissions: []
})
const formError = ref({})
...
const initData = () => {
    ...
    let rolePermissions = form.value.permissions.filter(function (item) {
        return props.additional.role_has_permissions.includes(item.id);
    })
    rolePermissions.forEach(function (part, index) {
...
const submit = async () => {
    ...
    axios.put(route('settings.systems.role.updateRole', {'id': props.additional.role.id}), form.value)
        .then((res) => {
            discard()
            notify({
                type: "success",
            ...
            Inertia.visit(route('settings.systems.role.index'))
        }).catch((res) => {
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-start space-x-2 items-center">
        <Link :href="route('settings.systems.role.index')" class="cursor-pointer">
            <VBack width="32" height="32" :is-active="backActive" @mouseover="backActive = true" @mouseout="backActive = false"/>
        </Link>
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Edit Role</h1>
    </div>
            ...
            <section>
                <div class="w-full">
                    ...
                    <VInput placeholder="Insert Role Name" label="Role Name" :required="true" v-model="form.role_name"
                        :errorMessage="formError.role_name" @update:modelValue="formError.role_name = ''" class="w-full sm:w-1/3 md:w-1/4 lg:w-1/5"/>
                </div>
            </section>
