...
const heads = ["NPP Name", "Company Registration Number", "Minimum Value", "Valid Month", "Status", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('settings.company.bpjs.bpjsk.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('settings.company.bpjs.bpjsk.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
    ...
    <section>
        <header class="block justify-between items-center sm:flex pb-6">
            <h2 class="font-semibold text-slate-800">
                All BPJS Kesehatan <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
            </h2>
            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                <!-- Filter -->
                <VFilter @search="searchHandle" />
                <VButton label="Add BPJSK" type="primary" @click="handleAddModalForm" class="mt-auto"/>
            </div>
        </header>
