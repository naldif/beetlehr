...
const createHandle = async () => {
    ...
    axios.post(route('settings.leave.type.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
const updateHandle = async () => {
    isLoading.value = true
    axios.put(route('settings.leave.type.update', { 'id': props.data.id }), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
        ...
        <template v-slot:content>
            <div class="grid grid-cols-1 gap-3">
                <VSelect placeholder="Select Branch" :required="true" v-model="form.branch_id"
                    ...
                    @update:modelValue="formError.branch_id = ''" ref="branchSelectHandle" :disabled="updateAction" />
