...
const createHandle = async () => {
    ...
    axios.post(route('settings.overtime.rule.create'), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
...
const updateHandle = async () => {
    isLoading.value = true
    axios.put(route('settings.overtime.rule.update', { 'id': props.data.id }), form.value)
        .then((res) => {
            emit('close')
            emit('successSubmit')
            form.value = ref({})
    ...
    <VDialog :showModal="openDialog" :title="updateAction ? 'Update Leave Type' : 'Add Leave Type'" @opened="openForm" @closed="closeForm" size="md">
        ...
        <template v-slot:content>
            <div class="grid grid-cols-1 gap-3">
                <div>
                    <label class="block text-sm font-medium text-slate-600 mb-1">Minimum Clockin <span class="text-rose-500">*</span>
                    </label>
                    <Datepicker v-model="form.clock_in" @update:modelValue="formError.clock_in = ''" time-picker
                        position="left" :clearable="false" placeholder="Minimum in (Hours:Minutes)"
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-600 mb-1">Maximum Clockout <span class="text-rose-500">*</span>
                    </label>
                    <Datepicker v-model="form.clock_out" @update:modelValue="formError.clock_out = ''" time-picker
                        position="left" :clearable="false" placeholder="Maximum in (Hours:Minutes)"
