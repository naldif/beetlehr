...
<template>
    ...
    <VFilter align="right" @apply="applyFilter" @clear="clearFilter">
        <div class="grid grid-cols-2 gap-3 px-4 pb-4 pt-1.5">
            <div>
                <div class="text-xs font-semibold text-slate-400 uppercase mb-1">Sort By Name</div>
                <VRadio name="minimum-value" v-model="filter.sort_by_name" :options="sortByNameOptions" />
            </div>
            <div>
                <div class="text-xs font-semibold text-slate-400 uppercase mb-1">Filter by Designation</div>
                <VSelect placeholder="Select Designation" v-model="filter.filter_designation" :options="additional.designation_list"
                    :clearable="false" ref="filterDesignation" />
            </div>
            <div>
                <div class="text-xs font-semibold text-slate-400 uppercase mb-1">Filter by Role</div>
                <VSelect placeholder="Select Role" v-model="filter.filter_role" :options="additional.role_list"
                    :clearable="false" ref="filterRole" />
            </div>
        </div>
    </VFilter>
</template>