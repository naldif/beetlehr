...
const breadcrumb = [
    ...
    },
    {
        name: "Employee",
        active: false,
        to: route('employment.resign-management.index')
    },
    {
        name: "Resign Management",
        active: true,
        to: route('employment.resign-management.index')
    },
]
...
const heads = ["Name", "Branch", "Date Submission", "End Contract", "Status", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('employment.resign-management.getdata'), {
        params: {
            page: page,
...
const deleteEmployee = async () => {
    axios.delete(route('employment.resign-management.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-between items-center">
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Resign Management</h1>
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200"
        ...
        <header class="block justify-between items-center sm:flex py-6 px-4">
            <h2 class="font-semibold text-slate-800">
                All Resign Submissions <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
            </h2>
            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                ...
                <VButton label="Add Resign" type="primary" @click="handleAddModalForm" class="mt-auto" />
            </div>
        </header>
        ...
        <VDataTable :heads="heads" :isLoading="isLoading">
            ...
            <tr v-for="(data, index) in query" :key="index" v-else>
                ...
                <td class="px-4 whitespace-nowrap h-16"> {{ data.date_submission }} </td>
