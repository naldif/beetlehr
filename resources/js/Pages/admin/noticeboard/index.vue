...
const heads = ["Title", "Branch", "Type", "End Date", ""]
...
const getData = debounce(async (page) => {
    axios.get(route('notice-board.getdata'), {
        params: {
            page: page,
...
const deleteHandle = async () => {
    axios.delete(route('notice-board.delete', { 'id': itemSelected.value.id })
    ).then((res) => {
        notify({
            type: "success",
            group: "top",
...
<template>
    ...
    <div class="mb-4 sm:mb-6 flex justify-between items-center">
        <h1 class="text-2xl md:text-3xl text-slate-800 font-bold">Notice Board</h1>
        <VSelect placeholder="Select Branch" v-model="filterBranchValue" :options="additional.branches_filter"
    ...
    <div class="bg-white shadow-lg rounded-sm border border-slate-200"
        ...
        <header class="block justify-between items-center sm:flex py-6 px-4">
            <h2 class="font-semibold text-slate-800">
                All Notice Boards <span class="text-slate-400 !font-medium ml">{{ pagination.total }}</span>
            </h2>
            <div class="mt-3 sm:mt-0 flex space-x-2 sm:justify-between justify-end">
                ...
                <VButton label="Add Notice Board" type="primary" @click="handleAddModalForm" class="mt-auto" />
            </div>
        </header>
        ...
        <VDataTable :heads="heads" :isLoading="isLoading">
            ...
            <tr v-for="(data, index) in query" :key="index" v-else>
                <td class="px-4 whitespace-nowrap h-16"> {{ data.title }} </td>
                <td class="px-4 whitespace-nowrap h-16"> {{ data.branch }} </td>
                <td class="px-4 whitespace-nowrap h-16 capitalize"> {{ data.type }} </td>
                <td class="px-4 whitespace-nowrap h-16"> {{ data.end_date }} </td>
